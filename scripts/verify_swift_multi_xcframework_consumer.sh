...
# Products/<configuration>/include directory. Two artifacts that both contain
# Headers/module.modulemap then race to produce the same output. This fixture
# combines Iroh with a generated flat XCFramework and fails if Iroh's module
# map ever escapes its product-scoped static framework.
...
arch -"$HOST_ARCH" xcrun clang \
  -arch "$HOST_ARCH" \
  -isysroot "$SDK" \
  -mmacosx-version-min=14.0 \
  -c "$TMP/other_flat.c" \
  -o "$TMP/other_flat.o"
arch -"$HOST_ARCH" xcrun libtool -static \
  -o "$TMP/libother_flat.a" \
  "$TMP/other_flat.o"
...
[ -f "$IROH_BINARY" ] || {
  echo "ERROR: static framework binary not found at $IROH_BINARY" >&2
  exit 1
}
